...
class RGPageViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIToolbarDelegate {
        ...
        if let tabViewContent: UIView = datasource?.tabViewForPageAtIndex(pageViewController: self, index: index) {
            ...
            case .Left:
                if let theHeight: CGFloat = delegate?.heightForTabAtIndex?(index: index) {
                    tabView = RGTabView(frame:
                        CGRect(x: 0.0, y: 0.0, width: tabbarWidth, height: theHeight), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalLeft)
            ...
            case .Right:
                if let theHeight: CGFloat = delegate?.heightForTabAtIndex?(index: index) {
                    ...
                    tabView = RGTabView(frame:
                        CGRect(x: 0.0, y: 0.0, width: tabbarWidth, height: tabbarWidth), indicatorColor: tabIndicatorColor, indicatorHW: tabIndicatorWidthOrHeight, style: tabStyle, orientation: .VerticalRight)
    ...
    private func updatePager(index: Int) {
        if let vc: UIViewController = viewControllerAtIndex(index: index) {
  ...
  func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
    ...
    private func viewControllerAtIndex(index: Int) -> UIViewController? {
        ...
        if (pageViewControllers.object(at: index) as AnyObject).isEqual(NSNull()), let vc: UIViewController = datasource?.viewControllerForPageAtIndex(pageViewController: self, index: index) {
...
class RGTabBarItem: UIView {
    ...
    func initSelf() {
        ...
        if let txt = text {
            ...
            textLabel!.frame =
                CGRect(x: 0.0, y: frame.size.height - (textLabel?.frame.size.height)! - 3.0, width: frame.size.width, height: (textLabel?.frame.size.height)!)
